...
The Application doesn't requiere any user personal info in order to access the full features of the application. The users may have the chance to add new content to the Application to extend it by adding new answers for the game which are not added yet to the Application database in the server.

## 3. LEGITIMATION

The Application will use the data introduced by the user to create new content for the game.
Once the Application is uninstalled from the user device, the data will be no longer availabl.

## 4. CONSERVATION OF PERSONAL DATA

The Application doesn't store personal info about their users, neither in the server nor locally.

## 5. RECIPIENTS

The data stored by the device will only be available in that device and will not be available for the rest of the users.
